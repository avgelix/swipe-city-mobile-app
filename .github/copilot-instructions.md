...
1. **Zero-cost MVP:** Must stay within free tiers (OpenRouter free model, Vercel free tier)
...
5. **Dynamic questions (Phase 2):** Neighborhood questions generated by AI based on matched city
...
2. **AI Matching Phase:** Send answers to Vercel function → OpenRouter AI generates city match + explanation
3. **Results Phase:** Display match with explanation; user accepts → Phase 2, or refuses → collect feedback → restart
4. **Neighborhood Phase:** AI generates city-specific neighborhood questions based on matched location (e.g., Tokyo: "Prefer Shibuya vs Asakusa?", NYC: "Manhattan vs Brooklyn?")
...
3. **Vercel Serverless Function:**
   - "Create a Vercel serverless function that proxies OpenRouter API calls keeping the API key secure"
   - "Build an API route that accepts user answers as JSON and returns city match from OpenRouter"
...
```
api/
  gemini.js  # Serverless function that proxies OpenRouter API calls
              # Accepts POST with user answers, returns city match
              # Keeps OPENROUTER_API_KEY in Vercel env variables
```

### Testing AI Integration Locally
- Use `vercel dev` to test serverless functions locally
- Mock OpenRouter responses during development to avoid rate limits
- Add console logging for API call monitoring
...
3. **OpenRouter API key security:** NEVER commit to Git. Always use Vercel environment variables + serverless function proxy.
...
- `api/gemini.js` - **COMPLETED** - Vercel serverless function for OpenRouter API proxy
...
### Configuration
- Environment variables in Vercel: `OPENROUTER_API_KEY`, `VITE_MAPBOX_TOKEN`
